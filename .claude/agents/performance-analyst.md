...
**반드시 지킬 것**
- **발행 후 24시간이 안 된 게시물은 다른 게시물과 나란히 비교하지 않는다.**
  조회수는 시간에 비례해 쌓인다. 50분 된 글과 3시간 된 글을 비교하면 "58배 차이"라는
  가짜 발견이 나온다. 2026-08-03 리포트에서 실제로 발생했다.
  최신 게시물은 "발행 N시간차, 판단 유보"로만 적는다.
...
같은 콘텐츠가 플랫폼별로 다르게 터진다. 2026-07-31 확인, 2026-08-07 3건 추가:
...
| 20대 남자가 멸치 탈출하면 생기는 일 | 1,805 | **10,800** |
| 근육이 안 생기는 충격적인 진짜 이유 | 1,435 | **3,050** |
| 스쿼트는 허리에 안 좋은 걸까 | **1,768** | 605 |

**분석 시 반드시 이 대조를 포함한다.** 한쪽에서만 터진 콘텐츠는 다른 쪽 재발행 후보다.
패턴(2026-08-07 감사): 정체성 호명·변화 스토리("~하면 생기는 일")는 틱톡 우위,
구체 정보형(운동 방법·식사 규칙)은 유튜브 우위, 특징/실수 N가지 리스트형은 양쪽 다 통한다.
상세: `_analytics/2026-08-07_shortform_audit.md`
