...
## 맥 자동 기상 — Claude 작업을 0원으로 유지

초안 생성·주간 리포트·텔레그램 명령은 `claude` CLI가 필요하고,
그건 맥의 **구독 로그인**으로 돌아간다. 서버로 옮기면 API 키가 필요해지고 과금이 시작된다.

옮기는 대신 **맥이 스스로 깨어나게** 한다. 구독 안에서 도니 추가 비용이 0원이다.

```bash
sudo pmset repeat wakeorpoweron MTWRFSU 08:00:00
```

매일 08:00 기상 → 08:13 초안 생성 → 월요일이면 09:07 리포트까지 돌고 다시 잠든다.

| 조건 | |
|------|---|
| 전원 | **연결돼 있어야 한다.** 배터리만으로는 예약 기상이 동작하지 않는다 |
| 상태 | 완전 종료가 아니라 **잠자기**. 덮개는 닫아도 된다 |
| 확인 | `pmset -g sched` 에 `wake at ... 08:00` 이 보이면 성공 |
| 해제 | `sudo pmset repeat cancel` |

두 작업 모두 네트워크가 올라올 때까지 최대 10분 기다리므로,
기상 직후 와이파이가 늦게 붙어도 실패하지 않는다.
...
| 어디서 | 무엇을 | 비용 |
|--------|--------|------|
| 오라클 | 승인 워커 (버튼·예약 발행) | **0원** — Always Free · Claude 미사용 |
| 맥 (자동 기상) | 초안·리포트·텔레그램 명령 | **0원** — 구독 안에서 실행 |
| 사람 | 유튜브 촬영 · 블로그 붙여넣기 · 승인 | — |

---

## 3단계 — API 키로 전부 서버화 (지금은 안 한다)
