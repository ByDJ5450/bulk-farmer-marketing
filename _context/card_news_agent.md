...
- [ ] 첫 슬라이드 훅: 고객 상황 직접 언급 (`_context/first_line_hooks.md` 패턴·체크리스트 적용)
- [ ] 커버에 "안에 뭐가 있는지" 한 줄 약속 포함 — 서론 금지 (`_context/sns_copy_guide.md` PART 3)
- [ ] 중간 슬라이드: 라벨 + 짧은 설명, 한 장에 포인트 하나 (두 가지 섞지 않기)
- [ ] 6장 이상이면 장 번호 표시 (예: 3/7)
- [ ] 마지막 슬라이드: @bulk_farmer 태그 + CTA (DM 또는 링크트리) — "뭘 하면 뭘 받는지" 구체적으로
- [ ] 캡션: 가장 센 줄을 맨 위로 (접힌 상태에서 보이는 첫 줄 승부)
