...
트레이너의 실제 벌크업 경험(55kg → 88kg, K-Classic 클래식 보디빌딩 3위)을 기반으로,
...
**텔레그램 보고** — 원본 초안 메시지를 고치기만 하면 채팅 위로 올라가 안 보인다.
그래서 워커가 별도 알림을 보낸다.

| 시점 | 내용 |
|------|------|
| 발행될 때마다 | ✅ 발행 완료 시각 · 소재 요약 · 글 링크 · **다음 예약 시각** · 남은 대기 수 |
| 23시 이후 하루 한 번 | 🌙 오늘 발행 개수. 3개 미만이면 승인을 더 하라고 짚는다 |
...
**원칙: 주간 원천 소재는 롱폼 1개다.** 새로 만드는 건 롱폼뿐이고 나머지는 전부 거기서 파생시킨다.
쇼츠는 롱폼 컷, 카드뉴스·블로그는 대본과 영상 프레임에서 나온다.

| 채널 | 주간 목표 | 포맷 | 출처 |
|------|---------|------|------|
| **유튜브 롱폼** | **1개** | 10~14분 | 촬영 — 유일한 병목 |
| 유튜브 쇼츠 | 3~5개 | 60초 이내 | 롱폼 컷 추출 |
| 틱톡 | 3~5개 | 15~60초 | 쇼츠 교차 |
| **인스타그램 릴스** | **3~5개** | 9:16 · 30초 권장 | 롱폼 컷 + 자막 |
| 인스타그램 카드뉴스 | 1개 | 1080×1350 캐러셀 | 롱폼 1건 |
...
>
> **쇼츠 개수를 더 올리지 않는 이유.** 같은 리포트 결론이 "발행량으로는 회복되지 않는다,
> 훅 유형을 바꿔야 한다"였다. 정체성 호명형 훅으로 4주 돌려 중앙값이 움직이는지 먼저 본다.
>
> **2026-08-03 조정 — 카드뉴스 2개 → 릴스 3~5개 + 카드뉴스 1개.**
> 인스타 실측(2026-08-03): 릴스 중앙값 **286**(n=21) vs 피드 중앙값 **133**(n=5).
> 도달 상위 7개 중 6개가 릴스다. 비팔로워 도달에서 릴스가 우위다.
>
> 직전 판단("카드뉴스는 성과가 검증된 포맷")은 7/9 캐러셀 **한 건**(2,552)만 보고 내린
> 것이었다. 표본 1개로 포맷을 결정했다. 8/2·8/3 카드뉴스는 도달 58·53에 그쳤다.
>
> **릴스는 API로 발행하지 않는다.** API 발행 릴스는 인스타 음원 라이브러리를 못 쓴다.
> 영상 파일을 텔레그램으로 받아 앱에서 직접 올린다 — 트렌딩 오디오 쓰는 30초가
> 도달 차이를 만든다.

> **2026-08-02 조정.** 직전 목표(인스타·스레드 하루 5~10개)는 실적(인스타 월 1개)과
> 200배 벌어져 있어 판단 기준으로 쓸 수 없었다. 실행 가능한 수치로 교체했다.
